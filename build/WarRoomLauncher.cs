...
internal static class WarRoomLauncher
{
    ...
    private sealed class HostContext:ApplicationContext,IDisposable
    {
        private readonly HttpListener listener;private readonly Process launched;private readonly System.Windows.Forms.Timer poll;private bool shuttingDown;
        public HostContext(HttpListener listener,Process launched){this.listener=listener;this.launched=launched;poll=new System.Windows.Forms.Timer();poll.Interval=250;poll.Tick+=(s,e)=>CheckAppLifetime();poll.Start();if(launched==null){poll.Stop();BeginShutdown();}}
        private void CheckAppLifetime(){if(shuttingDown)return;try{if(launched==null||launched.HasExited){BeginShutdown();return;}launched.Refresh();if(launched.MainWindowHandle==IntPtr.Zero&&launched.TotalProcessorTime>TimeSpan.FromSeconds(1)){BeginShutdown();}}catch{BeginShutdown();}}
        private void BeginShutdown(){if(shuttingDown)return;shuttingDown=true;try{poll.Stop();}catch{}try{if(listener!=null&&listener.IsListening)listener.Stop();}catch{}try{if(listener!=null)listener.Close();}catch{}ExitThread();}
        protected override void ExitThreadCore(){try{poll.Stop();poll.Dispose();}catch{}try{if(listener!=null&&listener.IsListening)listener.Stop();}catch{}try{if(listener!=null)listener.Close();}catch{}try{if(launched!=null)launched.Dispose();}catch{}base.ExitThreadCore();}
