...
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; INSPECT_MARK='/* War Room v1.7.26 - Stable Inspect enrichment publishes live character binding */'; GRAPHICS_MARK='/* War Room v1.7.25 - Live gems and enchantments in WoW tooltips */'; ARMORY_MARK='/* War Room v1.7.26 - Character model metadata + live gem and enchant normalization */'; MODEL_MARK='/* War Room v1.7.26 - Character model manifest builder */'; STAGE_MARK='/* War Room v1.7.26 - Isolated 3D character model stage */'
if CSS_MARK in h:
...
for marker in (CSS_MARK,JS_MARK,INSPECT_MARK,GRAPHICS_MARK,ARMORY_MARK,MODEL_MARK,STAGE_MARK,'WarRoomInspectLive','WarRoomCharacterModelManifest','WarRoomCharacterModelStage','wr-character-model-stage','resolveAppearance','/item-appearance?id=','directSlotLabels','found.length===1','/item-icon?id=','wr-live-item-icon','wr-wow-tooltip','bindTooltip','tooltipHtml','resolveLive','enrichLiveItem',"resolveLive('gem'", "resolveLive('enchantment'",'gemHtml','Enchant:','gemCache','enchantmentCache',"trinket2:'trinket ii'",'AVG ILVL','ROLE / SPEC','inflight','lastAttempt'):
...
if final.count(GRAPHICS_MARK)!=1 or final.count(ARMORY_MARK)!=1 or final.count(MODEL_MARK)!=1 or final.count(STAGE_MARK)!=1 or final.count(INSPECT_MARK)!=1: raise RuntimeError('Duplicate/missing v1.7.26 modules')
