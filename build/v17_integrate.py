...
h=index.read_text(encoding='utf-8'); css=(repo/'raid_intelligence.css').read_text(encoding='utf-8'); engine=(repo/'raid_intelligence.js').read_text(encoding='utf-8'); ui=(repo/'raid_intelligence_ui.js').read_text(encoding='utf-8')
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; UI_MARK='/* War Room v1.7 - live Raid Intelligence dashboard */'
if CSS_MARK in h:
...
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.3'; v['notes']=['Spec Data Confidence meter added beside raid composition','Fresh, aging, stale, unverified and missing talent/spec states tracked','Stale or missing specs reduce confidence in spec-dependent recommendations','Spec-dependent TBC coverage no longer assumes unknown talent data is correct','Side-by-side Raid Intelligence layout retained','TBC Phase 3 encounter-aware analysis retained']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
final=index.read_text(encoding='utf-8')
for marker in (CSS_MARK,JS_MARK,UI_MARK,'wrRaidIntelLive','WarRoomRaidIntelligence','wrRaidCommandDock','SPEC DATA CONFIDENCE','wrSpecMeta','wrDataQuality'):
    if marker not in final: raise RuntimeError(f'Missing v1.7.3 integration marker: {marker}')
if final.count(JS_MARK)!=1 or final.count(UI_MARK)!=1: raise RuntimeError('Duplicate Raid Intelligence modules detected after upgrade')
