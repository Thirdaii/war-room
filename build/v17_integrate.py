...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.14',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.14 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.14'; v['notes']=['Inspect automatically enriches any raider on demand without Refresh Current Raid','Single-character Inspect refresh hydrates specialization, item level, talents and equipment','Successful Inspect enrichment is cached to avoid unnecessary repeated armory requests','Bulk Refresh Current Raid remains available for raid-wide intelligence updates','TLS-enabled native armory proxy and diagnostics retained']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,REFRESH_MARK,ARMORY_MARK,UI_MARK,CHAR_MARK,INSPECT_MARK,'WarRoomArmoryRefresh','WarRoomSpecRefresh','renderWarRoomCharacterData','syncWarRoomInspectEnrichment','refreshNames([name]','Loading armory data','wr-character-data','/armory?name=','classicarmory.gg','Armory bridge'):
    if marker not in final: raise RuntimeError(f'Missing v1.7.14 integration marker: {marker}')
if final.count(JS_MARK)!=1 or final.count(UI_MARK)!=1 or final.count(REFRESH_MARK)!=1 or final.count(ARMORY_MARK)!=1 or final.count(CHAR_MARK)!=1 or final.count(INSPECT_MARK)!=1: raise RuntimeError('Duplicate v1.7.14 modules detected after upgrade')
...
print('War Room v1.7.14 Inspect auto enrichment integration complete')
