...
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; INSPECT_MARK='/* War Room v1.7.21 - Inspect enrichment publishes live character binding */'; GRAPHICS_MARK='/* War Room v1.7.25 - Live gems and enchantments in WoW tooltips */'; ARMORY_MARK='/* War Room v1.7.25 - Live gem and enchantment normalization */'
# Accept the branch module's original wording while keeping the canonical v1.7.25 marker for validation.
...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.25',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.25 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.25'; v['notes']=['Adds live equipped enchantments to Inspect item tooltips','Adds live socketed gems with resolved gem artwork and tooltip data','Preserves raw gem/enchantment IDs and metadata from armory equipment payloads','Caches gem and enchantment resolution in both browser and native host','Preserves working gear artwork, WoW hover tooltips, and persistent local-host launcher']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,INSPECT_MARK,GRAPHICS_MARK,ARMORY_MARK,'WarRoomInspectLive','directSlotLabels','found.length===1','/item-icon?id=','wr-live-item-icon','wr-wow-tooltip','bindTooltip','tooltipHtml','resolveLive','enrichLiveItem',"resolveLive('gem'", "resolveLive('enchantment'",'gemHtml','Enchant:','gemCache','enchantmentCache'):
 if marker not in final: raise RuntimeError(f'Missing v1.7.25 integration marker: {marker}')
...
if final.count(GRAPHICS_MARK)!=1 or final.count(ARMORY_MARK)!=1: raise RuntimeError('Duplicate/missing v1.7.25 modules')
...
print('War Room v1.7.25 live gems and enchantments integration complete')
