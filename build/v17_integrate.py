...
if not re.fullmatch(r'\d+\.\d+\.\d+',version): raise RuntimeError(f'Invalid build version: {version!r}')
...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE '+version,h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v'+version+' • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']=version; v['notes']=['Preserves live equipped enchantments, socketed gems, gear artwork, and WoW tooltips','Keeps native app-mode launcher and clean local-host shutdown','Build version is sourced from build/version.txt so integration cannot silently downgrade a release']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,INSPECT_MARK,GRAPHICS_MARK,ARMORY_MARK,'WarRoomInspectLive','directSlotLabels','found.length===1','/item-icon?id=','wr-live-item-icon','wr-wow-tooltip','bindTooltip','tooltipHtml','resolveLive','enrichLiveItem',"resolveLive('gem'", "resolveLive('enchantment'",'gemHtml','Enchant:','gemCache','enchantmentCache'):
 if marker not in final: raise RuntimeError(f'Missing v1.7 integration marker: {marker}')
...
if final.count(GRAPHICS_MARK)!=1 or final.count(ARMORY_MARK)!=1: raise RuntimeError('Duplicate/missing integration modules')
...
if version not in final: raise RuntimeError('Integrated UI lost requested build version')
print(f'War Room v{version} integration complete')
