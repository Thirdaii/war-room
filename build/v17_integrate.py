...
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; INSPECT_MARK='/* War Room v1.7.21 - Inspect enrichment publishes live character binding */'; GRAPHICS_MARK='/* War Room v1.7.24 - WoW-style item hover tooltips */'
if CSS_MARK in h:
...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.24',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.24 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.24'; v['notes']=['Adds WoW-style hover tooltips to live Inspect equipment','Shows verified rarity, item level, slot/type, stats, sockets, enchants, effects, requirements, durability and set data when supplied','Positions tooltips intelligently around viewport edges','Preserves v1.7.22 exact equipment cards and live gear artwork','Preserves v1.7.23 native local-host launcher behavior']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,INSPECT_MARK,GRAPHICS_MARK,'WarRoomInspectLive','directSlotLabels','found.length===1','/item-icon?id=','wr-live-item-icon','wr-wow-tooltip','bindTooltip','tooltipHtml','positionTooltip','mouseenter','mousemove','mouseleave','qualityColor','Item Level','sockets','enchantment'):
 if marker not in final: raise RuntimeError(f'Missing v1.7.24 integration marker: {marker}')
...
if final.count(GRAPHICS_MARK)!=1: raise RuntimeError('Duplicate/missing v1.7.24 tooltip module')
...
print('War Room v1.7.24 WoW item tooltip integration complete')
