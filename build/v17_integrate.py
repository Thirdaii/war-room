...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.16',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.16 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.16'; v['notes']=['Inspect equipment matrix supports live item icons from armory metadata','Item quality now drives visual border and glow styling','Equipment header identifies live armory equipment instead of awaiting feed','Average item level, specialization, talents and item data remain unchanged','TLS-enabled native armory proxy and raid intelligence retained']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,REFRESH_MARK,ARMORY_MARK,UI_MARK,CHAR_MARK,INSPECT_MARK,GRAPHICS_MARK,'WarRoomArmoryRefresh','WarRoomSpecRefresh','syncWarRoomInspectEnrichment','applyWarRoomInspectGraphics','wr-live-item-icon','wow.zamimg.com','icon:x.icon','classicarmory.gg'):
    if marker not in final: raise RuntimeError(f'Missing v1.7.16 integration marker: {marker}')
if final.count(JS_MARK)!=1 or final.count(UI_MARK)!=1 or final.count(REFRESH_MARK)!=1 or final.count(ARMORY_MARK)!=1 or final.count(CHAR_MARK)!=1 or final.count(INSPECT_MARK)!=1 or final.count(GRAPHICS_MARK)!=1: raise RuntimeError('Duplicate v1.7.16 modules detected after upgrade')
...
print('War Room v1.7.16 Inspect graphics integration complete')
