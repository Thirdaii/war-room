...
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; REFRESH_MARK='/* War Room v1.7.8 - Spec Refresh Bridge */'; ARMORY_MARK='/* War Room v1.7.12 - ClassicArmory Proxy Diagnostics */'; UI_MARK='/* War Room v1.7 - live Raid Intelligence dashboard */'; CHAR_MARK='/* War Room v1.7.12 - Raid Character Data Diagnostics */'; INSPECT_MARK='/* War Room v1.7.21 - Inspect enrichment publishes live character binding */'; GRAPHICS_MARK='/* War Room v1.7.21 - live-bound gear cards + lifecycle-safe artwork */'
if CSS_MARK in h:
...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.21',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.21 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.21'; v['notes']=['Binds Inspect equipment graphics directly to the hydrated armory character that populated the dossier','Publishes a lifecycle-scoped warroom:inspect-character-ready handoff after verified equipment loads','Restores item names and item levels before icon resolution','Keeps real 64x64 TBC item artwork and native item-ID icon fallback','Preserves detached/stale Inspect lifecycle protections']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
for marker in (CSS_MARK,JS_MARK,REFRESH_MARK,ARMORY_MARK,UI_MARK,CHAR_MARK,INSPECT_MARK,GRAPHICS_MARK,'WarRoomArmoryRefresh','syncWarRoomInspectEnrichment','applyWarRoomInspectGraphics','WarRoomInspectLive','warroom:inspect-character-ready','boundCharacter','/item-icon?id=','LIVE ARMORY EQUIPMENT','wr-live-item-icon','activeScope','generation','width:64px','hidePlaceholderArt','renderItemText','dataset.wrSlot'):
    if marker not in final: raise RuntimeError(f'Missing v1.7.21 integration marker: {marker}')
...
if final.count(JS_MARK)!=1 or final.count(INSPECT_MARK)!=1 or final.count(GRAPHICS_MARK)!=1: raise RuntimeError('Duplicate v1.7.21 modules detected after upgrade')
...
print('War Room v1.7.21 live-bound Inspect gear integration complete')
