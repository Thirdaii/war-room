...
CSS_MARK='/* War Room v1.7 Raid Intelligence */'; JS_MARK='/* War Room v1.7 - TBC Phase 3 Raid Intelligence */'; REFRESH_MARK='/* War Room v1.7.8 - Spec Refresh Bridge */'; ARMORY_MARK='/* War Room v1.7.12 - ClassicArmory Proxy Diagnostics */'; UI_MARK='/* War Room v1.7 - live Raid Intelligence dashboard */'; CHAR_MARK='/* War Room v1.7.12 - Raid Character Data Diagnostics */'; INSPECT_MARK='/* War Room v1.7.18 - Inspect lifecycle-safe enrichment */'; GRAPHICS_MARK='/* War Room v1.7.19 - lifecycle-safe real gear artwork */'
if CSS_MARK in h:
...
h=re.sub(r'WAR ROOM APP • (?:COMMAND CENTER 1\.6|RAID INTELLIGENCE 1\.7(?:\.[0-9]+)?)','WAR ROOM APP • RAID INTELLIGENCE 1.7.19',h,count=1)
h=re.sub(r'WAR ROOM v1\.(?:6|7(?:\.[0-9]+)?) • (?:NATIVE LAUNCHER|TBC RAID INTELLIGENCE)','WAR ROOM v1.7.19 • TBC RAID INTELLIGENCE',h,count=1)
v=json.loads(version_file.read_text(encoding='utf-8')); v['version']='1.7.19'; v['notes']=['Inspect renders real 64x64 TBC item artwork as the primary equipment-slot visual','Generic slot art is hidden only after the real item image successfully loads','Item-quality framing remains applied around equipped gear','v1.7.18 lifecycle protections remain intact for Inspect open/close behavior','Native item-ID icon resolver remains the fallback when armory icon metadata is missing']; version_file.write_text(json.dumps(v,indent=2),encoding='utf-8'); index.write_text(h,encoding='utf-8')
if updater.exists():
...
if 'scope.isConnected' not in inspectui or 'scope!==activeScope' not in graphics or 'generation++' not in graphics: raise RuntimeError('v1.7.18 Inspect lifecycle guards were lost')
...
print('War Room v1.7.19 visible gear artwork integration complete')
